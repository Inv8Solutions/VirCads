...
export class Scene62 extends Scene {
    ...
    create() {
        ...
        g.strokeRoundedRect(dlgX - dlgW / 2, dlgY - dlgH / 2, dlgW, dlgH, 12);

        const dlgStyle = { fontSize: '20px', color: '#000', fontFamily: 'Arial', align: 'center', wordWrap: { width: dlgW - 40 } } as any;
        ...
        const dlX = dlgX - gap;
        const dlBtn = this.add.rectangle(dlX, btnY, 220, 52, 0x388e3c, 1).setDepth(202).setInteractive({ useHandCursor: true });
        dlBtn.setStrokeStyle(3, 0x000000, 1);
        ...
        const nxX = dlgX + gap;
        const nxBtn = this.add.rectangle(nxX, btnY, 160, 52, 0x3B82F6, 1).setDepth(202).setInteractive({ useHandCursor: true });
        nxBtn.setStrokeStyle(3, 0x000000, 1);
