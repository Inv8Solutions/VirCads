...
export class Scene58 extends Scene {
    ...
    create() {
        if (this.textures.exists('scene_58')) {
            const bg = this.add.image(800, 450, 'scene_58');
            bg.setDisplaySize(1600, 900);
            bg.setDepth(0);
        } else if (this.textures.exists('scene_58')) {
            const bg = this.add.image(800, 450, 'scene_58');
            bg.setDisplaySize(1600, 900);
            bg.setDepth(0);
        } else {
        ...
        const PIXELS_PER_CM = Math.hypot(968 - 962, 596 - 592) / 0.3;
        let measuring = false;
        ...
        this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
            console.log(`[INPUT] Scene58 click screen=(${pointer.x},${pointer.y}) world=(${pointer.worldX},${pointer.worldY})`);
                    ...
                    if (!doneButton) {
                        ...
                        doneButton.on('pointerdown', () => {
                            ...
                            this.cameras.main.once('cameraflashcomplete', () => {
                                ...
                                // OK button to dismiss and go to Scene59
                                const okBtnX = 800;
                                const okBtnY = 520;
