...
export class Scene29 extends Scene {
    ...
    create() {
        ...
        const panelX = 800;
        const panelY = 450;
        const panel = this.add.rectangle(panelX, panelY, panelW, panelH, 0x000000, 0.75).setDepth(50);
        ...
        const tip = this.add.text(panelX - panelW/2 + 24, panelY - panelH/2 + 24, tipText, { fontSize: '26px', color: '#ffffff', lineSpacing: 6, wordWrap: { width: panelW - 48 } }).setDepth(51).setOrigin(0,0);

        const continueText = this.add.text(panelX, panelY + panelH/2 - 36, 'Click here to continue', { fontSize: '30px', color: '#ffffff', backgroundColor: '#0066cc', padding: { x: 20, y: 14 } }).setOrigin(0.5).setDepth(52);
